...
// Configurar volume (30% para não atrapalhar)
...
audioToggle.addEventListener('click', () => {
    if (isMusicPlaying) {
        ...
        backgroundMusic.play().catch(e => {
            console.log('Áudio precisa de interação do usuário primeiro');
        });
        audioIcon.textContent = '🔊';
...
// Tentar tocar áudio quando o usuário interagir pela primeira vez
document.addEventListener('click', () => {
    if (!isMusicPlaying && backgroundMusic.paused) {
        backgroundMusic.play().then(() => {
            ...
            // Áudio só toca após interação explícita
        });
    }
}, { once: true });
...
function startGame() {
    ...
    if (backgroundMusic.paused && !isMusicPlaying) {
        backgroundMusic.play().then(() => {
            ...
            // Se falhar, o usuário pode clicar no botão manualmente
        });
    }
...
function resetGame() {
    ...
    // Reiniciar música se estiver tocando
    if (isMusicPlaying && backgroundMusic.paused) {
